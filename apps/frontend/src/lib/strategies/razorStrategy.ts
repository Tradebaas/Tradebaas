...
import type { ErrorLog, Candle } from '@/types/shared';
...
export class RazorStrategy {
  ...
  private onPositionClose?: (trade: ActiveTrade, pnl: number, reason: string) => void;
  ...
  constructor(
    ...
    callbacks?: {
      ...
      onPositionClose?: (trade: ActiveTrade, pnl: number, reason: string) => void;
  ...
  private buildMechanicalCheckpoints() {
    ...
    if (this.candleHistory.length < 20) {
      checkpoints.push({
        id: 'data-loading',
        label: 'Data laden',
        description: 'Wachten op voldoende marktdata',
        status: 'pending',
        value: `${this.candleHistory.length}/20 candles`,
        details: 'Minimaal 20 candles nodig voor analyse',
      });
      return checkpoints;
    }
    ...
    if (!bb5 || rsi3 === undefined) {
      checkpoints.push({
        id: 'indicator-error',
        label: 'Indicator berekening',
        description: 'Fout bij berekenen indicatoren',
        status: 'not-met',
        details: 'Kon BB of RSI niet berekenen',
      });
      return checkpoints;
    }
    ...
    const volumeConfirmed = volumeSpike > 1.2;
    const momentumNegative = momentum < -0.1;
    const momentumPositive = momentum > 0.1;

    checkpoints.push({
      id: 'price-bb',
      label: 'Prijs raakt Bollinger Band',
      ...
      details: priceTouchesLowerBB 
        ? `Raakt lower BB ($${bb5.lower.toFixed(2)}) - Long setup mogelijk`
        : priceTouchesUpperBB
        ? `Raakt upper BB ($${bb5.upper.toFixed(2)}) - Short setup mogelijk`
        : `Tussen bands | Lower: $${bb5.lower.toFixed(2)} | Upper: $${bb5.upper.toFixed(2)}`,
    });

    checkpoints.push({
      id: 'rsi-extreme',
      label: 'RSI(3) in extreem gebied',
      description: 'RSI < 25 (oversold) of RSI > 75 (overbought)',
      status: rsiOversold || rsiOverbought ? 'met' : 'not-met',
      value: `${rsi3.toFixed(1)}`,
      details: rsiOversold 
        ? 'Oversold - Long setup mogelijk'
        : rsiOverbought
        ? 'Overbought - Short setup mogelijk'
        : `Neutraal bereik (tussen 25-75)`,
    });

    checkpoints.push({
      id: 'volume-spike',
      label: 'Volume bevestiging',
      description: 'Volume moet > 1.2x gemiddelde zijn',
      status: volumeConfirmed ? 'met' : 'not-met',
      value: `${volumeSpike.toFixed(2)}x`,
      details: volumeConfirmed 
        ? `Volume spike gedetecteerd (${volumeSpike.toFixed(2)}x gemiddelde)`
        : `Volume te laag (${volumeSpike.toFixed(2)}x, minimaal 1.2x nodig)`,
    });

    checkpoints.push({
      id: 'momentum',
      label: 'Momentum richting',
      description: 'Momentum < -0.1% (long) of > +0.1% (short)',
      status: momentumNegative || momentumPositive ? 'met' : 'not-met',
      value: `${momentum > 0 ? '+' : ''}${momentum.toFixed(2)}%`,
      details: momentumNegative
        ? `Negatief momentum (-${Math.abs(momentum).toFixed(2)}%) - Long setup mogelijk`
        : momentumPositive
        ? `Positief momentum (+${momentum.toFixed(2)}%) - Short setup mogelijk`
        : `Momentum te zwak (${momentum.toFixed(2)}%, minimaal ±0.1% nodig)`,
    });
  ...
  private async scanForOpportunities(): Promise<void> {
    ...
    if (!this.client.isConnected()) {
      this.logger.warn('[Razor] WebSocket not connected, skipping scan');
      return;
    }

    try {
      const positionsRaw = await this.client.request('private/get_positions', { currency: 'USDC' });
      const positions: DeribitPosition[] = Array.isArray(positionsRaw) ? positionsRaw : [];
      const hasPosition = positions.some((pos) => 
        pos.instrument_name === this.config.instrument && (pos.size || 0) !== 0
      );
      ...
      if (errorMsg.includes('WebSocket not connected')) {
        this.logger.warn('[Razor] WebSocket disconnected during scan');
        return;
      }
  ...
  if (result && Array.isArray(result.ticks) && Array.isArray(result.open)) {
  ...
  freshEquity = typeof balance?.equity === 'number' ? balance.equity : this.equity;
    } catch (error) {
  ...
  const orderState = await this.client.request('private/get_order_state', {
        ...
        if (orderState.order_state === 'filled') {
          ...
          actualPosition = positions.find((pos) => 
    ...
    try {
      ...
      const openOrders: Array<{ order_id: string; order_type?: string; trigger?: unknown }> =
    ...
    for (let attempt = 1; attempt <= this.config.maxBracketRetries; attempt++) {
      try {
        ...
        const roundedSL = this.roundToTick(signal.stopPrice, instrument.tick_size);
        this.logger.info('[Razor] ✅ SL placed', { 
          orderId: slOrderId,
          trigger_price: roundedSL,
          raw: signal.stopPrice,
        });

        await new Promise(resolve => setTimeout(resolve, 300));

        const tpDirection = signal.side === 'buy' ? 'sell' : 'buy';
        const roundedTP = this.roundToTick(signal.takeProfitPrice, instrument.tick_size);
        ...
        this.logger.info('[Razor] ✅ TP placed', { 
          orderId: tpOrderId,
          price: roundedTP,
          raw: signal.takeProfitPrice,
        });
    ...
    this.logger.info('[Razor] ✅✅✅ TRADE FULLY EXECUTED WITH PROTECTION', {
      ...
      tpOrderId,
      verification: {
        slRounded: this.roundToTick(signal.stopPrice, instrument.tick_size),
        tpRounded: this.roundToTick(signal.takeProfitPrice, instrument.tick_size),
        tickSize: instrument.tick_size,
      },
    });
  ...
  private async closeTrade(tradeId: string, exitPrice: number, reason: string): Promise<void> {
    ...
    if (this.onPositionClose) {
      this.onPositionClose(trade, pnl, reason);
    }
