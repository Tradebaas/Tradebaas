// Dynamic backend URL - uses same host as frontend, port 3000
const getBackendUrl = () => {
...
export class BackendClient {
  ...
  private async loadJWT() {
    try {
      ...
      if (!window.spark?.kv) {
        console.warn('[BackendClient] Spark KV not available, skipping JWT load');
        return;
      }

      const jwt = await window.window.spark.kv.get<string>('entitlement_jwt');
      if (jwt) {
  ...
  private async saveJWT(jwt: string) {
    this.entitlementJWT = jwt;
    if (window.spark?.kv) {
      await window.spark.kv.set('entitlement_jwt', jwt);
    }
  }
  ...
  async connect(brokerId: string, credentials: BackendCredentials): Promise<ConnectResponse> {
    try {
      ...
      if (data.success) {
        ...
        await window.spark.kv.set('connected_broker', brokerId);
      }
  ...
  async getBrokerName(): Promise<string | null> {
    ...
    try {
      ...
      const stored = await Promise.race([
        window.spark.kv.get<string>('connected_broker'),
