...
export class AdvancedBracketManager {
  ...
  async attachInitialBracket(args: {
    ...
    try {
      ...
      const triggerOrderCount = openOrders?.filter((o: any) => 
    ...
    try {
      ...
      const tp1Order = await this.client.request('private/' + tp1Direction, {
        instrument_name: this.symbol,
        amount: tp1Qty,
        type: 'limit',
        price: this.roundToTick(oneRPrice),
  ...
  private async moveSLToBreakeven(): Promise<void> {
    ...
    try {
      ...
      const newSlOrder = await this.client.request('private/' + slDirection, {
        instrument_name: this.symbol,
        amount: halfQty,
        type: 'stop_market',
        trigger: 'mark_price',
