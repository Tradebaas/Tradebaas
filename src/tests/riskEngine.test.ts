...
describe('Risk Engine - Position Sizing', () => {
  const defaultBrokerRules: BrokerRules = createBrokerRules(50, 0.5, 10, 10);
  ...
  describe('calculatePosition - Valid Scenarios', () => {
    ...
    it('should respect broker max leverage cap', () => {
      const strictRules = createBrokerRules(10, 0.5, 10, 10);
      const input: RiskEngineInput = {
        equity: 100,
        riskMode: 'fixed',
        riskValue: 50,
    ...
    it('should round quantity to lot size', () => {
      const rules = createBrokerRules(50, 0.5, 5, 5);
      const input: RiskEngineInput = {
        equity: 1000,
        riskMode: 'fixed',
        riskValue: 50,
  ...
  describe('Broker-Specific Rules', () => {
    it('should handle Deribit rules (50x leverage)', () => {
      const deribitRules = createBrokerRules(50, 0.5, 10, 10);
      const input: RiskEngineInput = {
        equity: 1000,
        riskMode: 'percent',
        riskValue: 2,
    ...
    it('should handle Kraken rules (5x leverage)', () => {
      const krakenRules = createBrokerRules(5, 0.1, 0.001, 0.001);
      const input: RiskEngineInput = {
        equity: 1000,
        riskMode: 'percent',
        riskValue: 10,
    ...
    it('should handle Binance rules (125x leverage)', () => {
      const binanceRules = createBrokerRules(125, 0.1, 1, 1);
      const input: RiskEngineInput = {
        equity: 1000,
        riskMode: 'percent',
        riskValue: 1,
