...
export function CurrentPositionCard() {
  ...
  useEffect(() => {
    if (!position || !currentPrice || !entryPrice) return;

    const analyzeMarket = async () => {
      if (analyzingMarket) return;
      
      setAnalyzingMarket(true);
      try {
        const isLong = position.direction === 'buy';
        const pnl = isLong ? currentPrice - entryPrice : entryPrice - currentPrice;
        const pnlPercent = (pnl / entryPrice) * 100;
        
        const distanceToSL = stopLoss ? Math.abs(currentPrice - stopLoss) : 0;
        const distanceToTP = takeProfit ? Math.abs(takeProfit - currentPrice) : 0;
        
        const priceMovement = isLong ? currentPrice > entryPrice : currentPrice < entryPrice;
        const movingTowardsProfit = priceMovement;

        const prompt = spark.llmPrompt`Je bent een trading analist die real-time marktbewegingen analyseert.

Huidige Positie:
- Richting: ${isLong ? 'LONG' : 'SHORT'}
- Instrument: ${position.instrument_name}
- Entry Prijs: $${entryPrice.toFixed(2)}
- Huidige Prijs: $${currentPrice.toFixed(2)}
- Stop Loss: ${stopLoss ? `$${stopLoss.toFixed(2)}` : 'GEEN'}
- Take Profit: ${takeProfit ? `$${takeProfit.toFixed(2)}` : 'GEEN'}
- Current P&L: ${pnlPercent.toFixed(2)}%
- Prijs beweegt richting winst: ${movingTowardsProfit ? 'JA' : 'NEE'}
- Afstand tot SL: ${distanceToSL > 0 ? `$${distanceToSL.toFixed(2)}` : 'N/A'}
- Afstand tot TP: ${distanceToTP > 0 ? `$${distanceToTP.toFixed(2)}` : 'N/A'}

Analyseer de huidige marktsituatie en bepaal:
1. De waarschijnlijkheid dat deze trade winstgevend wordt (0-100%)
2. Een korte analyse in het Nederlands waarom

Baseer je analyse op:
- Huidige prijs momentum (beweegt het mee met de positie?)
- Afstand tot SL en TP
- Current P&L status
- Risico dat de SL getriggerd wordt

Geef je antwoord in dit formaat:
{
  "probability": <getal tussen 0 en 100>,
  "analysis": "<2-3 zinnen analyse in het Nederlands>"
}`;
    ...
    analyzeMarket();
    const analysisInterval = setInterval(analyzeMarket, 30000);
  ...
  const handleClosePositionConfirm = async () => {
    ...
    try {
      ...
      const botToken = await window.spark.kv.get<string>('telegram-bot-token');
      const chatId = await window.spark.kv.get<string>('telegram-chat-id');
      const telegramEnabled = await window.spark.kv.get<string>('telegram-enabled');
