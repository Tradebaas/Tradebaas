...
export function StrategyTradingCard({ tradingBlocked = false, onOpenDisclaimer }: StrategyTradingCardProps) {
  ...
  // Determine actual strategy status (backend takes priority for 24/7)
  ...
  const handleStrategyToggle = async () => {
    ...
    if (isRunning) {
      ...
      // QUEUE logic: Position open, user wants to queue strategy
      // DON'T start immediately, just save selection for auto-resume
      if (selectedStrategy) {
        await window.spark.kv.set('last-active-strategy', selectedStrategy);
        
        // Mark strategy as queued (not stopped, will auto-resume)
        set({ 
          ...
          <div className="space-y-4 flex-shrink-0">
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="ema-rsi-scalper">EMA-RSI Scalper</SelectItem>
                  <SelectItem value="fast-test-strategy">Fast Test</SelectItem>
                  <SelectItem value="third-iteration">Vortex</SelectItem>
            ...
            <Button
              ...
              disabled={
                tradingBlocked || 
                !isConnected || 
                (!selectedStrategy && (strategy === null && !isStrategyRunningOnBackend && !activePosition))
              }
            ...
            {actualStrategyStatus === 'paused' && activePosition && (
