...
// Add current server IP for development
const DEV_URLS = ['http://217.154.69.143:5000', 'http://217.154.69.143:5173', 'http://217.154.69.143:5174'];
const ALLOWED_ORIGINS = process.env.NODE_ENV === 'production' 
  ? PRODUCTION_URLS 
  : [...FRONTEND_URLS, ...PRODUCTION_URLS, ...DEV_URLS];
...
server.get('/api/strategy/status', async (request) => {
  ...
  const strategies = strategyService.getStrategyStatus(strategyId);
  return {
    success: true,
    strategies,
  ...
  try {
    ...
    if (!analysis) {
      return reply.code(404).send({
        success: false,
        error: 'No analysis data available for this strategy',
      });
    }
    return {
      success: true,
